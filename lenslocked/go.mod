module github.com/imeraj/go_playground/lenslocked

go 1.14

require (
	github.com/adelowo/muxlist v0.0.0-20181116150235-bc84a003bcc2
	github.com/cosmos72/gomacro v0.0.0-20200607151955-647446f8352c // indirect
	github.com/fsnotify/fsnotify v1.4.7
	github.com/go-playground/locales v0.12.1
	github.com/go-playground/universal-translator v0.16.0
	github.com/go-playground/validator v9.29.1+incompatible
	github.com/go-sql-driver/mysql v1.5.0
	github.com/gorilla/mux v1.7.3
	github.com/gorilla/schema v1.1.0
	github.com/gorilla/securecookie v1.1.1
	github.com/hashicorp/hcl v1.0.0
	github.com/jinzhu/gorm v1.9.14
	github.com/jinzhu/inflection v1.0.0
	github.com/leodido/go-urn v1.1.0
	github.com/magiconair/properties v1.8.1
	github.com/mdempsky/gocode v0.0.0-20200405233807-4acdcbdea79d // indirect
	github.com/micro/micro/v2 v2.9.2-0.20200716110055-affce197e7bf // indirect
	github.com/mitchellh/mapstructure v1.1.2
	github.com/motemen/go-quickfix v0.0.0-20200118031250-2a6e54e79a50 // indirect
	github.com/motemen/gore v0.5.0 // indirect
	github.com/pelletier/go-toml v1.4.0
	github.com/spf13/afero v1.2.2
	github.com/spf13/cast v1.3.0
	github.com/spf13/jwalterweatherman v1.1.0
	github.com/spf13/pflag v1.0.3
	github.com/spf13/viper v1.4.0
	golang.org/x/crypto v0.0.0-20200709230013-948cd5f35899
	golang.org/x/sys v0.0.0-20200523222454-059865788121
	golang.org/x/text v0.3.3
	golang.org/x/tools v0.0.0-20200715235423-130c9f19d3fe // indirect
	google.golang.org/appengine v1.6.1
	gopkg.in/yaml.v2 v2.2.4
)
